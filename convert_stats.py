...
def postResult(n):
    cmd = 'curl -XPOST "http://localhost:9200/stepmania_stats/stats" -H "Content-Type: application/json" --data-binary @/Users/okada-toshiki/Desktop/stats/result0.json'
...
# 既存のインデックス情報を再作成
subprocess.call(
    'curl -XDELETE "http://localhost:9200/stepmania_stats/"', shell=True)
subprocess.call(
    'curl -XPUT "http://localhost:9200/stepmania_stats/" -H "Content-Type: application/json"', shell=True)

# 前回作成時のデータを削除
for path in glob.glob("/Users/okada-toshiki/Desktop/stats/*.json"):
    os.remove(path)
...
for path in glob.glob("/Users/okada-toshiki/Library/Preferences/StepMania 5/LocalProfiles/00000000/Stats.xml"):
    ...
    with open('/Users/okada-toshiki/Desktop/stats/result' + str(i) + '.json', 'a') as fw:
