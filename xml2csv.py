...
def list_all_files(inputDir):
    ...
    for (dirname, dirnames, filenames) in os.walk(inputDir):
        ...
        allFiles = allFiles + [os.path.join(dirname, filename)
    ...
    if os.path.isdir(inputfile):
        ...
        if outputfile is None:
            outputfile = os.path.abspath(
                os.path.join(inputfile, "..", "output.csv"))
        else:
        ...
        with open(outputfile, 'a') as outf:
            ...
            if header:
                with open(inputfiles[0], 'r') as f:
                    ...
                    taglist = [
                        tag.name if tag.string is not None else 'None' for tag in soup.find_all()]
            ...
            for inputfile in inputfiles:
                with open(inputfile, 'r') as f:
                    soup = BeautifulSoup(f)
                    itemlist = [
                        tag.string if tag.string is not None else 'None' for tag in soup.find_all()]
                    outf.write(",".join(itemlist) + "\n")
    # If the inputfile is a file
    else:
        if outputfile is None:
            outputfile = os.path.abspath(os.path.join(
                os.path.dirname(inputfile), "..", "output.csv"))
        else:
            ensure_output_dir(outputfile)
