...
def postResult(n):
    cmd = 'curl -XPOST "http://localhost:9200/stepmania/result" -H "Content-Type: application/json" --data-binary @/Users/okada-toshiki/Desktop/result/result' + \
        str(n) + '.json > /dev/null 2>&1'
...
subprocess.call(
    'curl -XPUT "http://localhost:9200/stepmania" -H "Content-Type: application/json" -d @stepmania_mapping.json', shell=True)
...
for path in glob.glob("/Users/okada-toshiki/Library/Preferences/StepMania 5/Upload/*.xml"):
    ...
    if len(root[1]) == 2:
        root[1][0].getparent().remove(root[1][0])
    ...
    if root[1][0][2][1].text == "Failed":
        root[1][0][2].set("Cleared", "false")
    else:
        root[1][0][2].set("Cleared", "true")
            ...
            for smline in smlines:
                ...
                if 'dance-single:' in smline:
                    if smlines[smline_num+2].rstrip(':\n').lstrip(' ') == root[1][0][1].attrib['Difficulty']:
                        root[1][0][1].set(
                            "Level", smlines[smline_num+3].rstrip(':\n').lstrip(' '))
                smline_num += 1
